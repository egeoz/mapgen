...
proc newMapGenerator*(width: int = 1920, height: int = 1080, output: string, biome: Biome = Temperate, noise: NoiseType = NoiseType.Simplex, noiseSeed: float = -0.2, iterationCount: int = 16, bottomElevation: uint8 = 185, lowElevation: uint8 = 170, midElevation: uint8 = 160, highElevation: uint8 = 145, topElevation: uint8 = 0): MapGenerator =
    result = MapGenerator(width: width, height: height, output: output, biome: biome, noise: noise, noiseSeed: noiseSeed, iterationCount: iterationCount, bottomElevation: bottomElevation, lowElevation: lowElevation, midElevation: midElevation, highElevation: highElevation, topElevation: topElevation)
...
proc convertToRGB(mapgen: MapGenerator, val: uint8): Pixel =
    var r, g, b: uint8
...
proc generateMap*(mapgen: MapGenerator) =
    ...
    for x in 0 ..< mapgen.width:
        for y in 0 ..< mapgen.height:
            mapSeq[mapgen.coordToPos(x, y)] = mapgen.convertToRGB(mapgen.iterateNoise(x, y))
...
proc saveMap*(mapgen: MapGenerator) =
    discard savePNG24(mapgen.output, mapSeq.serialize(), mapgen.width, mapgen.height)
